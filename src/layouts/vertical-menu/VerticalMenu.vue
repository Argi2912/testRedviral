    ...
    <div class="app-page flex-column flex-column-fluid" id="kt_app_page">
      ...
      <div id="kt_app_header" class="app-header">
        ...
        >
          ...
          <div
            class="d-flex align-items-stretch flex-row-reverse flex-lg-grow-1"
            ...
            <div class="app-navbar flex-shrink-0">
              ...
              <div class="app-navbar-item ms-1 ms-lg-3">
                ...
                <a
                  ...
                  :class="[mode_menu_open && 'show menu-dropdown']"
                  @click="mode_menu_open = !mode_menu_open"
                ...
                <div
                  ...
                  :class="[mode_menu_open && 'show mode-menu-open']"
                  data-kt-menu="true"
              ...
              >
                ...
                <div
                  ...
                  :class="[settings_open && 'show menu-dropdown']"
                ...
                <div
                  ...
                  :class="[settings_open && 'show settings-open']"
                  data-kt-menu="true"
                >
                  <!--begin::Menu item-->
      ...
      <div class="app-wrapper flex-column flex-row-fluid" id="kt_app_wrapper">
        ...
        >
          ...
          <div class="app-sidebar-logo px-6" id="kt_app_sidebar_logo">
            <!--begin::Logo image-->
            <a class="clickable" @click="pushToDashboard">
              <img
                alt="Logo"
            ...
            >
              ...
              >
                <div
                  ...
                  @click="displayAccordion('level_one', i)"
                  :style="`display:${
                    showItem(level_one_option.group_permissions)
                      ? 'block'
                      : 'block'
                  }`"
                  ...
                  :class="[levels.indexOf(i) != -1 && 'hover show']"
                >
                  <span
                    v-if="showItem(level_one_option.group_permissions)"
                    class="menu-link"
                    @click="redirect(level_one_option)"
                  >
                  ...
                  >
                    <div
                      ...
                      :style="`display:${
                        showItem(level_two_option.group_permissions)
                          ? 'block'
                          : 'none'
                      }`"
                      data-kt-menu-trigger="click"
                      class="menu-item hover showing"
                      :class="[level_two_option.items && 'menu-accordion']"
                    >
                      <!--begin:Menu link-->
                      <a
                        v-if="showItem(level_two_option.group_permissions)"
                        class="menu-link"
                        :style="`${
                          route.name == level_two_option.route_name
                            ? 'background:#2884ef'
                            : ''
                        }`"
                        @click="redirect(level_two_option)"
                      >
                        <span class="menu-bullet" style="color: rgba(255, 255, 255, 0.5)">
                          -
                        </span>
                        <span class="menu-title" style="color: rgba(255, 255, 255, 0.5)">{{
                      ...
                      >
                        ...
                        >
                          <a
                            v-if="
                              showItem(level_three_option.group_permissions)
                            "
                            class="menu-link"
                            :style="`${
                              route.name == level_two_option.route_name
                          ...
                          <span v-else
                            >No tiene permisos para los modulos de este
                            espacio</span
                          >
                        </div>
                      </div>
          ...
          >
            <a
              class="btn btn-flex flex-center btn-custom btn-primary overflow-hidden text-nowrap px-0 h-40px w-100 clickable"
              data-bs-toggle="tooltip"
              data-bs-trigger="hover"
              data-bs-dismiss-="click"
            >
              <span class="btn-label">Iniciar documentación</span>
...
<script setup>
import { computed, onBeforeMount, ref, reactive } from "vue-demi";
...
const options_exclude = ["create_transactions", "show_roles"];
const group_exclude_by_role = {
  Faqs: [2],
};

const showItem = (module_group_permission) => {
  let _user_permissions = auth.getProperty("role").permissions;

  let has_permission = _user_permissions.find((permission) => {
    if (typeof module_group_permission == "string") {
      let group_excluded_to_user = false;

      if (group_exclude_by_role[permission.group] != undefined) {
        group_excluded_to_user =
          group_exclude_by_role[permission.group].indexOf(
            auth.getProperty("user").role
          ) != -1;
      }
      return (
        !group_excluded_to_user &&
        permission.group == module_group_permission &&
        options_exclude.indexOf(permission.tag) == -1
      );
    } else {
      if (module_group_permission.length == 0) return true;
      return (
        module_group_permission.indexOf(permission.group) != -1 &&
        options_exclude.indexOf(permission.tag) == -1
      );
    }
  });
...
let options_hash_map = {
  2: client_options,
  3: worker_options,
};

const init = async () => {
  await auth.refreshProfileInfo();
  let user_role = auth.getUserProperty("role");
  console.log(user_role);
  console.log(options_hash_map[user_role]);
  options.value = options_hash_map[user_role] || admin_options;
};
...
let shown_options = reactive({
  level_one: false,
  level_two: false,
  level_three: false,
});
let levels = ref([]);
let settings_open = ref(false);
let mode_menu_open = ref(false);
const displayAccordion = (level, i) => {
  let _levels_index = levels.value.indexOf(i);
  if (_levels_index == -1) {
    levels.value.push(i);
  } else {
    levels.value.splice(_levels_index, 1);
  }
  if (level == "level_one") {
    shown_options.level_one = !shown_options.level_one;
    return;
  }
  if (level == "level_two") {
    shown_options.level_two = !shown_options.level_two;
    return;
  }
  if (level == "level_three") {
    shown_options.level_three = !shown_options.level_three;
    return;
  }
};
...
.mode-menu-open {
  ...
  inset: 0px 0px auto auto;
  margin: 0px;
  transform: translate3d(-79.5px, 70px, 0px);
}
</style>
