...
export default {
    ...
    update: async(data, payments_history_id) => {
    let auth = useAuthStore()
    let parsed_data = parseToFormData(data)
    return await Http.post({

        route: '/' + SERVICES_ROUTE + '/update/' + payments_history_id,
        data: parsed_data,
        headers: {
            Authorization: 'Bearer ' + auth.user.token
        }
...
delete: async(payments_history_id) => {
    let auth = useAuthStore()
    return await Http.delete({

        route: '/' + SERVICES_ROUTE + '/' + payments_history_id,
        headers: {
            Authorization: 'Bearer ' + auth.user.token
        }
    })
...
find: async(payments_history_id) => {
    let auth = useAuthStore()
