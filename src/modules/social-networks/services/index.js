...
export default {
  ...
  update: async (data, social_networks_id) => {
    let auth = useAuthStore();
    let parsed_data = parseToFormData(data);
    return await Http.post({
      route: "/" + SERVICES_ROUTE + "/update/" + social_networks_id,
      data: parsed_data,
      headers: {
        Authorization: "Bearer " + auth.user.token,
      },
  ...
  delete: async (social_networks_id) => {
    let auth = useAuthStore();
    return await Http.delete({
      route: "/" + SERVICES_ROUTE + "/" + social_networks_id,
      headers: {
        Authorization: "Bearer " + auth.user.token,
      },
    });
  },
  find: async (social_networks_id) => {
    let auth = useAuthStore();
