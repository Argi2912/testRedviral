...
export default {
  ...
  update: async (data, categories_id) => {
    let auth = useAuthStore();
    let parsed_data = parseToFormData(data);
    return await Http.post({
      route: "/" + SERVICES_ROUTE + "/update/" + categories_id,
      data: parsed_data,
      headers: {
        Authorization: "Bearer " + auth.user.token,
      },
  ...
  delete: async (categories_id) => {
    let auth = useAuthStore();
    return await Http.delete({
      route: "/" + SERVICES_ROUTE + "/" + categories_id,
      headers: {
        Authorization: "Bearer " + auth.user.token,
      },
    });
  },
  find: async (categories_id) => {
    let auth = useAuthStore();
