  ...
  <div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card">
      <div class="row mt-6 mx-6">
        <div class="col">
          <Forbidden
            :permission-tag="'list_categories'"
        ...
        <div class="col">
          ...
          >
            ...
            <!--begin::Add customer-->
            <Forbidden
              :permission-tag="'create_categories'"
              :user-permissions="auth_store.getProperty('role').permissions"
            >
              <button
      ...
      <div class="card-body">
        ...
        <Forbidden
          ...
          :user-permissions="auth_store.getProperty('role').permissions"
        >
          <div v-if="!categories_store.items.categories.loading_list">
            <div v-if="categories_store.items.categories.list.length > 0">
              <div class="table-responsive table-height">
                ...
                >
                  ...
                  <tbody class="fw-semibold text-gray-600">
                    <tr
                      v-for="(item, i) in categories_store.items.categories
                        .list"
                      :key="'item_' + i"
                    >
                      <!--begin::Checkbox-->
                        ...
                        >
                          <div class="dropdown">
                            ...
                            >
                              <li class="menu-item">
                                <Forbidden
                                  :permission-tag="'show_categories'"
                                  :user-permissions="
                                    auth_store.getProperty('role').permissions
                                  "
                                >
                              ...
                              <li class="menu-item">
                                <Forbidden
                                  :permission-tag="'update_categories'"
                                  :user-permissions="
                                    auth_store.getProperty('role').permissions
                                  "
                                >
                              ...
                              <li class="menu-item">
                                <Forbidden
                                  :permission-tag="'delete_categories'"
                                  :user-permissions="
                                    auth_store.getProperty('role').permissions
                                  "
                                >
              ...
              <div class="mt-6">
                ...
                <div style="float: right">
                  <Paginator
                    v-if="
                      Math.ceil(
                        categories_store.items.categories.paginator.total_pages
                      ) > 1
                    "
                    :key="categories_store.items.categories.paginator.key"
                    @onChangePage="onChangePage"
                    :items-per-page="
                      categories_store.items.categories.paginator.items_per_page
                    "
                    :max-buttons="5"
                    :total-items="
                      categories_store.items.categories.paginator.total_items
                    "
                    :total-pages="
                      Math.ceil(
                        categories_store.items.categories.paginator.total_pages
                    ...
                    "
                    :current-page="
                      categories_store.items.categories.paginator.current_page
                    "
                  />
                </div>
              </div>
    ...
    <HeaderModal
      ...
      :show="categories_store.modal != ''"
      :title="categories_store.modal_title"
      :is-static="true"
      :scrollable="true"
    >
      <template #body>
        <CategoriesForm
          v-if="categories_store.forms.create_update_categories != ''"
        />
      </template>
    </HeaderModal>
...
let category_type = ref("Todos");
const setTypeCateogoryFilter = async (selected_category_type) => {
  categories_store.items.categories.loading_list = true;
  if (selected_category_type == "Todos") {
    await categories_store.cleanSearchFilter("type_category");
  } else {
    await categories_store.addSearchFilter({
      field: "type_category",
      value: selected_category_type,
    });
  }
  await categories_store.setCategoriesList();
  categories_store.items.categories.loading_list = false;
};
watch(category_type, (v) => {
  setTypeCateogoryFilter(v);
});
const deleteMany = () => {
  confirm(
    swal,
...
const deleteCategories = (item, id) => {
  categories_store.setSelectedItem("categories", item);
  confirm(swal, "¿Está seguro que desea eliminar este elemento?", () => {
    categories_store.deleteCategories(id);
  });
};
...
const checkUncheckAll = (v) => {
  if (v.target.checked) {
    ...
    categories_store.items.categories.list.map((item) => {
...
const onChangePage = (a) => {
  categories_store.setPagination({
    paginator: "categories",
    current_page: a,
  });
  categories_store.setCategoriesList();
};
...
const paginateBySearch = async () => {
  categories_store.items.categories.loading_list = true;
  search.value == ""
    ? await categories_store.cleanSearchFilter()
    : await categories_store.addSearchFilter({
        field: "name",
        value: search.value,
      });
...
const createCategories = () => {
  categories_store.setModal("create_categories");
  categories_store.setModalTitle("Agregar categories");
  categories_store.setFormMode("create_update_categories", "create");
};
const seeCategories = (item, index) => {
  categories_store.setModal("see_categories");
  categories_store.setModalTitle("Ver datos de categories");
  categories_store.setFormMode("create_update_categories", "see");
  categories_store.setSelectedItem("categories", item);
  categories_store.setSelectedItemIndex("categories", index);
};

const updateCategories = (item, index) => {
  categories_store.setModal("update_categories");
  categories_store.setModalTitle("Editar categories");
  categories_store.setFormMode("create_update_categories", "update");
  categories_store.setSelectedItem("categories", item);
  categories_store.setSelectedItemIndex("categories", index);
};

const closeCategoriesModal = () => {
  categories_store.setModal("");
  categories_store.setModalTitle("");
  categories_store.setFormMode("create_update_categories", "");
  categories_store.setSelectedItem("categories", {});
  categories_store.setSelectedItemIndex("categories", -1);
};
</script>
