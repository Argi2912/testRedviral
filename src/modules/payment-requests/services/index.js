...
export default {
    ...
    update: async(data, payment_requests_id) => {
    let auth = useAuthStore()
    let parsed_data = parseToFormData(data)
    return await Http.post({

        route: '/' + SERVICES_ROUTE + '/update/' + payment_requests_id,
        data: parsed_data,
        headers: {
            Authorization: 'Bearer ' + auth.user.token
        }
...
delete: async(payment_requests_id) => {
    let auth = useAuthStore()
    return await Http.delete({

        route: '/' + SERVICES_ROUTE + '/' + payment_requests_id,
        headers: {
            Authorization: 'Bearer ' + auth.user.token
        }
    })
...
find: async(payment_requests_id) => {
    let auth = useAuthStore()
