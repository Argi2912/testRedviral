  ...
  >
    ...
    >
      ...
      <div class="page-title d-flex flex-column justify-content-center me-3">
        ...
        >
          {{ payment_requests_store.module_name }}
        </h1>
        <!--end::Title-->
        <!--begin::Breadcrumb-->
        <ul
  ...
  <div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="!payment_requests_store.items.payment_requests.loading_list">
          <div
            v-if="payment_requests_store.items.payment_requests.list.length > 0"
          >
              ...
              <table class="table table-row-dashed align-middle gs-0 gy-3 my-0">
                ...
                <thead>
                  ...
                  >
                    ...
                    <th class="min-w-125px">Valor a pagar</th>
                ...
                <tbody class="fw-semibold text-gray-600">
                  <tr
                    v-for="(item, i) in payment_requests_store.items
            ...
            <div class="mt-6">
              ...
              <div style="float: right">
                <Paginator
                  v-if="
                    Math.ceil(
                      payment_requests_store.items.payment_requests.paginator
                        .total_pages
                    ) > 1
                  "
                  :key="
                    payment_requests_store.items.payment_requests.paginator.key
                  "
    ...
    <HeaderModal
      @onClose="closePaymentRequestsModal()"
      :show="payment_requests_store.modal != ''"
      :title="payment_requests_store.modal_title"
      :is-static="true"
      :scrollable="true"
    >
      <template #body>
...
import PaymentRequestsForm from "./PaymentRequestsForm.vue";
...
const deletePaymentRequests = (item, id) => {
  payment_requests_store.setSelectedItem("payment_requests", item);
  confirm(swal, "¿Está seguro que desea eliminar este elemento?", () => {
    payment_requests_store.deletePaymentRequests(id);
  });
};
...
const checkUncheckAll = (v) => {
  if (v.target.checked) {
    ...
    payment_requests_store.items.payment_requests.list.map((item) => {
...
const onChangePage = (a) => {
  payment_requests_store.setPagination({
    paginator: "payment_requests",
    current_page: a,
  });
  payment_requests_store.setPaymentRequestsList();
};
let items_per_page = ref(
  payment_requests_store.items.payment_requests.paginator.items_per_page
);
...
const paginateBySearch = async () => {
  payment_requests_store.items.payment_requests.loading_list = true;
  search.value == ""
    ? await payment_requests_store.cleanSearchFilter()
    : await payment_requests_store.addSearchFilter({
        field: "name",
        value: search.value,
      });
  payment_requests_store.setPagination({
...
const createPaymentRequests = () => {
  payment_requests_store.setModal("create_payment_requests");
  payment_requests_store.setModalTitle("Agregar payment_requests");
  payment_requests_store.setFormMode(
    "create_update_payment_requests",
    "create"
  );
};
const seePaymentRequests = (item, index) => {
  payment_requests_store.setModal("see_payment_requests");
  payment_requests_store.setModalTitle("Ver datos de payment_requests");
  payment_requests_store.setFormMode("create_update_payment_requests", "see");
  payment_requests_store.setSelectedItem("payment_requests", item);
  payment_requests_store.setSelectedItemIndex("payment_requests", index);
};

const payRequests = (item, index) => {
  payment_requests_store.setModal("update_payment_requests");
  payment_requests_store.setModalTitle("Editar payment_requests");
  payment_requests_store.setFormMode(
    "create_update_payment_requests",
    "update"
  );
...
const closePaymentRequestsModal = () => {
  payment_requests_store.setModal("");
  payment_requests_store.setModalTitle("");
  payment_requests_store.setFormMode("create_update_payment_requests", "");
  payment_requests_store.setSelectedItem("payment_requests", {});
  payment_requests_store.setSelectedItemIndex("payment_requests", -1);
};
</script>
