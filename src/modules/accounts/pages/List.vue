<template>
  ...
  <div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card">
      <div class="row mt-6 mx-6">
        ...
        <div class="col">
          ...
          >
            ...
            <!--begin::Add customer-->
            <Forbidden
              :permission-tag="'create_accounts'"
              :user-permissions="auth_store.getProperty('role').permissions"
            >
              <button
      ...
      <div class="card-body">
        ...
        <Forbidden
          ...
          :user-permissions="auth_store.getProperty('role').permissions"
        >
          <div v-if="!accounts_store.items.accounts.loading_list">
            <div v-if="accounts_store.items.accounts.list.length > 0">
              <div class="table-responsive table-height">
                ...
                >
                  ...
                  <tbody class="fw-semibold text-gray-600">
                    <tr
                      v-for="(item, i) in accounts_store.items.accounts.list"
                      :key="'item_' + i"
                    >
                      <!--begin::Checkbox-->
                      <td>
                      ...
                      <td class="text-end">
                        <Forbidden
                          text="No tienes permisos para usar las opciones"
                          text-align="right"
                          :permission-tags="[
                            'show_accounts',
                            'delete_accounts',
                        ...
                        >
                          <div class="dropdown">
                            ...
                            >
                              ...
                              <Forbidden
                                :permission-tag="'update_accounts'"
                                :user-permissions="
                                  auth_store.getProperty('role').permissions
                                "
                              >
                              ...
                              <Forbidden
                                :permission-tag="'delete_accounts'"
                                :user-permissions="
                                  auth_store.getProperty('role').permissions
                                "
                              >
                                <li class="menu-item">
                                  <a
                                    href="#"
                                    class="menu-link px-3"
                                    @click="deleteAccounts(item, item.id)"
                                    >Eliminar</a
                                  >
                                </li>
                              </Forbidden>
              ...
              <div class="mt-6">
                ...
                <div style="float: right">
                  <Paginator
                    v-if="
                      Math.ceil(
                        accounts_store.items.accounts.paginator.total_pages
                      ) > 1
                    "
                    :key="accounts_store.items.accounts.paginator.key"
                    @onChangePage="onChangePage"
                    :items-per-page="
                      accounts_store.items.accounts.paginator.items_per_page
                    "
                    :max-buttons="5"
                    :total-items="
                      accounts_store.items.accounts.paginator.total_items
                    "
                    :total-pages="
                      Math.ceil(
                        accounts_store.items.accounts.paginator.total_pages
                    ...
                    "
                    :current-page="
                      accounts_store.items.accounts.paginator.current_page
                    "
                  />
                </div>
              </div>
    ...
    <HeaderModal
      ...
      :show="accounts_store.modal != ''"
      :title="accounts_store.modal_title"
      :is-static="true"
      :scrollable="true"
    >
      <template #body>
        <AccountsForm
          v-if="accounts_store.forms.create_update_accounts != ''"
        />
      </template>
    </HeaderModal>
...
const deleteAccounts = (item, id) => {
  accounts_store.setSelectedItem("accounts", item);
  confirm(swal, "¿Está seguro que desea eliminar este elemento?", () => {
    accounts_store.deleteAccounts(id);
  });
};
...
const checkUncheckAll = (v) => {
  if (v.target.checked) {
    ...
    accounts_store.items.accounts.list.map((item) => {
...
const onChangePage = (a) => {
  accounts_store.setPagination({
    paginator: "accounts",
    current_page: a,
  });
  accounts_store.setAccountsList();
};
...
const paginateBySearch = async () => {
  accounts_store.items.accounts.loading_list = true;
  search.value == ""
    ? await accounts_store.cleanSearchFilter()
    : await accounts_store.addSearchFilter({
        field: "name",
        value: search.value,
      });
  accounts_store.setPagination({
...
const createAccounts = () => {
  accounts_store.setModal("create_accounts");
  accounts_store.setModalTitle("Agregar accounts");
  accounts_store.setFormMode("create_update_accounts", "create");
};
const seeAccounts = (item, index) => {
  accounts_store.setModal("see_accounts");
  accounts_store.setModalTitle("Ver datos de accounts");
  accounts_store.setFormMode("create_update_accounts", "see");
  accounts_store.setSelectedItem("accounts", item);
  accounts_store.setSelectedItemIndex("accounts", index);
};

const updateAccounts = (item, index) => {
  accounts_store.setModal("update_accounts");
  accounts_store.setModalTitle("Editar accounts");
  accounts_store.setFormMode("create_update_accounts", "update");
  accounts_store.setSelectedItem("accounts", item);
  accounts_store.setSelectedItemIndex("accounts", index);
};

const closeAccountsModal = () => {
  accounts_store.setModal("");
  accounts_store.setModalTitle("");
  accounts_store.setFormMode("create_update_accounts", "");
  accounts_store.setSelectedItem("accounts", {});
  accounts_store.setSelectedItemIndex("accounts", -1);
};
</script>
