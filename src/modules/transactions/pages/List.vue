<template>
  ...
  >
    ...
    >
      ...
      <div class="page-title d-flex flex-column justify-content-center me-3">
        ...
        >
          {{ transactions_store.module_name }}
        </h1>
        <!--end::Title-->
        <!--begin::Breadcrumb-->
        <ul
  ...
  <div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card">
      <div class="row mt-6 mx-6">
        ...
        <div class="col">
          ...
          >
            ...
            <div
              class="dropdown"
              v-if="auth_store.getUserProperty('role') == 1"
            >
              <button
      ...
      <div class="card-body">
        <Forbidden
          show-image
          text="No tiene permiso para listar las transacciones."
          :permission-tag="'list_transactions'"
          :user-permissions="auth_store.getProperty('role').permissions"
        >
          <div v-if="!transactions_store.items.transactions.loading_list">
            <div v-if="transactions_store.items.transactions.list.length > 0">
                ...
                >
                  ...
                  <thead>
                    ...
                    >
                      ...
                      <th class="min-w-125px">Transacción</th>
                  ...
                  <tbody class="fw-semibold text-gray-600">
                    <tr
                      v-for="(item, i) in transactions_store.items.transactions
                        .list"
                      :key="'item_' + i"
                    >
                      <!--begin::Checkbox-->
                      ...
                      <td>
                        <a
                          ...
                          ><Badge
                            ...
                            "
                            :background="
                              item.operation == 'Pago' ? 'green' : 'blue'
                            "
                        /></a>
                      </td>
                      <!--end::Email=-->
                      ...
                      <td>{{ numberToCurrency("COP", item.amount) }}</td>
                      <!--end::Payment method=-->
                      <!--begin::Date=-->
                      ...
                      >
                        <div class="dropdown">
                          ...
                          >
                            <li
                              class="menu-item"
                              v-if="item.operation == 'Pago'"
                            >
                              <a
                                href="#"
                                @click="seeTransactions(item, i)"
                                class="menu-link px-3"
                                >Detalles</a
                              >
                            </li>
              ...
              <div class="mt-6">
                ...
                <div style="float: right">
                  <Paginator
                    ...
                    "
                    :key="transactions_store.items.transactions.paginator.key"
  ...
  <HeaderModal
    ...
    :show="transactions_store.modal != ''"
    :title="transactions_store.modal_title"
    :is-static="true"
    :scrollable="true"
  >
    <template #body>
      <div v-if="transactions_store.modal != ''">
        <div class="row">
          <div class="col">
            <b>Trabajador:</b>
            {{ transactions_store.items.transactions.selected_item.user.name }}
            {{
              transactions_store.items.transactions.selected_item.user.lastname
            }}
            <br />
            <b>Monto:</b>
            {{
              numberToCurrency(
                "COP",
                transactions_store.items.transactions.selected_item.amount
              )
            }}
            <br />
            <b>Método de pago:</b>
            {{
              transactions_store.items.transactions.selected_item.payment_method
            }}
            <br />
            <span
              v-if="
                transactions_store.items.transactions.selected_item
                  .payment_method == 'AirTM'
              "
            >
              <b>Email de AirTm:</b>
              {{
                transactions_store.items.transactions.selected_item.user.airtm
              }}
            </span>
            <span v-else>
              <b>Email de Binance:</b>
              {{
                transactions_store.items.transactions.selected_item.user.binance
              }}
            </span>
            <a
              class="clickable"
              v-if="
                transactions_store.items.transactions.selected_item.state !=
                'Pagado'
              "
              @click="tryPay"
            >
...
let type = ref("Todos");
const filterByType = async (operation) => {
  type.value = operation == "Devolución" ? "Solicitudes" : operation;
  if (operation == "Todos") {
    await transactions_store.cleanSearchFilter("operation");
  } else {
    await transactions_store.addSearchFilter({
      field: "operation",
      value: operation == "Devolución" ? operation : "Pago",
    });
  }
  transactions_store.setTransactionsList();
};
...
const tryPay = () => {
  confirm(swal, "¿Está seguro que desea aceptar este pago?", async () => {
    await transactions_store.updateTransactions(
      {
        state: "Pagado",
      },
      transactions_store.items.transactions.selected_item.id
    );
    transactions_store.setModal("");
    transactions_store.setModalTitle("");
    transactions_store.setSelectedItem("transactions", {});
  });
};

const rayzarPayment = (transactionsId) => {
  try { 
  confirm(swal, "¿Está seguro que desea rechazar este pago?", async () => {
    await transactions_store.rayzarPayment(
      {
        state: "Devolución",
      },
      transactionsId
...
const markAsPaid = (item) => {
  transactions_store.setModal("paid_devolution");
  transactions_store.setModalTitle("Pagar solicitud");
  transactions_store.setSelectedItem("transactions", item);
};

const checkUncheckAll = (v) => {
  if (v.target.checked) {
    selected_items.value = [];
    transactions_store.items.transactions.list.map((item) => {
...
const createTransactions = () => {
  transactions_store.setFormMode("create_update_transactions", "create");
  emit("onDisplayPage", {
    page_to_show: "#create-update-transactions",
  });
};
const seeTransactions = (item, index) => {
  transactions_store.setFormMode("create_update_transactions", "update");
  transactions_store.setSelectedItem("transactions", item);
  transactions_store.setSelectedItemIndex("transactions", index);
  emit("onDisplayPage", {
    page_to_show: "#transactions-details",
  });
};
...
const closeTransactionsModal = () => {
  transactions_store.setModal("");
  transactions_store.setModalTitle("");
  transactions_store.setFormMode("create_update_transactions", "");
  transactions_store.setSelectedItem("transactions", {});
  transactions_store.setSelectedItemIndex("transactions", -1);
};
</script>
<script>
export default {
