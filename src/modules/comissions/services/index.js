...
export default {
    ...
    update: async(data, comissions_id) => {
    let auth = useAuthStore()
    let parsed_data = parseToFormData(data)
    return await Http.post({

        route: '/' + SERVICES_ROUTE + '/update/' + comissions_id,
        data: parsed_data,
        headers: {
            Authorization: 'Bearer ' + auth.user.token
        }
...
delete: async(comissions_id) => {
    let auth = useAuthStore()
    return await Http.delete({

        route: '/' + SERVICES_ROUTE + '/' + comissions_id,
        headers: {
            Authorization: 'Bearer ' + auth.user.token
        }
    })
