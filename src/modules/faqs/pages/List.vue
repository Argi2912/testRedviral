  ...
  <div id="kt_app_content" class="app-content flex-column-fluid">
    ...
    <div class="card">
      ...
      <div class="card-body">
        <Forbidden
          show-image
          text="No tiene permisos para listar las preguntas frecuentes"
          :permission-tag="'list_faqs'"
          :user-permissions="auth_store.getProperty('role').permissions"
        >
          <!--begin::Table container-->
          <div v-if="!faqs_store.items.faqs.loading_list">
            <div v-if="faqs_store.items.faqs.list.length > 0">
                ...
                >
                  ...
                  <thead>
                    ...
                    >
                      ...
                      <th class="min-w-125px">Pregunta</th>
                  ...
                  <tbody class="fw-semibold text-gray-600">
                    <tr
                      v-for="(item, i) in faqs_store.items.faqs.list"
                      :key="'item_' + i"
                    >
                      <!--begin::Checkbox-->
                      <td>
              ...
              <div class="mt-6">
                ...
                <div style="float: right">
                  <Paginator
                    v-if="
                      Math.ceil(faqs_store.items.faqs.paginator.total_pages) > 1
                    "
                    :key="faqs_store.items.faqs.paginator.key"
                    @onChangePage="onChangePage"
                    :items-per-page="
                      faqs_store.items.faqs.paginator.items_per_page
                    "
                    :max-buttons="5"
                    :total-items="faqs_store.items.faqs.paginator.total_items"
                    :total-pages="
                      Math.ceil(faqs_store.items.faqs.paginator.total_pages)
                    "
                    :current-page="faqs_store.items.faqs.paginator.current_page"
                  />
                </div>
              </div>
            </div>
    ...
    <HeaderModal
      ...
      :show="faqs_store.modal != ''"
      :title="faqs_store.modal_title"
      :is-static="true"
      :scrollable="true"
    >
      <template #body>
        <FaqsForm v-if="faqs_store.forms.create_update_faqs != ''" />
      </template>
    </HeaderModal>
...
const deleteFaqs = (item, id) => {
  faqs_store.setSelectedItem("faqs", item);
  confirm(swal, "¿Está seguro que desea eliminar este elemento?", () => {
    faqs_store.deleteFaqs(id);
  });
};
...
const checkUncheckAll = (v) => {
  if (v.target.checked) {
    ...
    faqs_store.items.faqs.list.map((item) => {
...
const onChangePage = (a) => {
  faqs_store.items.faqs.loading_list = true;
  faqs_store.setPagination({
    paginator: "faqs",
    current_page: a,
  });
  faqs_store.setFaqsList();
};
let items_per_page = ref(faqs_store.items.faqs.paginator.items_per_page);
...
const paginateBySearch = async () => {
  faqs_store.items.faqs.loading_list = true;
  search.value == ""
    ? await faqs_store.cleanSearchFilter("name")
    : await faqs_store.addSearchFilter({
        field: "name",
        value: search.value,
      });
  faqs_store.setPagination({
...
const createFaqs = () => {
  faqs_store.setModal("create_faqs");
  faqs_store.setModalTitle("Agregar faqs");
  faqs_store.setFormMode("create_update_faqs", "create");
};
const seeFaqs = (item, index) => {
  faqs_store.setModal("see_faqs");
  faqs_store.setModalTitle("Ver datos de faqs");
  faqs_store.setFormMode("create_update_faqs", "see");
  faqs_store.setSelectedItem("faqs", item);
  faqs_store.setSelectedItemIndex("faqs", index);
};

const updateFaqs = (item, index) => {
  faqs_store.setModal("update_faqs");
  faqs_store.setModalTitle("Editar faqs");
  faqs_store.setFormMode("create_update_faqs", "update");
  faqs_store.setSelectedItem("faqs", item);
  faqs_store.setSelectedItemIndex("faqs", index);
};
...
const closeFaqsModal = () => {
  faqs_store.setModal("");
  faqs_store.setModalTitle("");
  faqs_store.setFormMode("create_update_faqs", "");
  faqs_store.setSelectedItem("faqs", {});
  faqs_store.setSelectedItemIndex("faqs", -1);
};
</script>
